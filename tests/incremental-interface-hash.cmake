...
assert_not_compiled("${out}" MainDiamond "unchanged interface must not recompile App")
assert_exit("${WORK_DIR}/build/diamond/diamond" 23)
...
fixture_build(out)
assert_build_ok(out)
assert_compiled("${out}" Base "edited Base must recompile")
if(HASH_PRECISE)
  assert_not_compiled("${out}" Left "body-only edit must not recompile Left")
  assert_not_compiled("${out}" MainDiamond "body-only edit must not recompile App")
else()
  assert_compiled("${out}" Left "conservative hash must recompile Left")
  assert_compiled("${out}" MainDiamond "conservative hash must recompile App")
endif()
...
fixture_build(out)
assert_build_ok(out)
assert_compiled("${out}" Left "interface change must recompile importer")
assert_compiled("${out}" MainDiamond "interface change must recompile transitive dependent")
assert_exit("${WORK_DIR}/build/diamond/diamond" 24)
...
fixture_build(out)
assert_build_ok(out)
assert_compiled("${out}" Left "interface removal must recompile importer")
assert_compiled("${out}" MainDiamond "interface removal must recompile transitive dependent")
assert_exit("${WORK_DIR}/build/diamond/diamond" 24)
