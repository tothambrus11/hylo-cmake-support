...
```cmake
find_package(Hylo 0.0.8 REQUIRED)                  # cmake/FindHylo.cmake on CMAKE_MODULE_PATH
...
- **`examples/`** — hello (a minimal multi-file module), two-module program,
  module graph (diamond), C interop, plain `add_library` + `hylo_target_module`.
...
Requires CMake ≥ 3.30 and hc ≥ 0.0.8 (`hc` on `PATH`, or `-DHylo_COMPILER=…`).
